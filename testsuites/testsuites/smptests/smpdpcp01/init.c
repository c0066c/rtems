...
#include <rtems/score/smplock.h>
const char rtems_test_name[] = "SMPDPCP 1";
...
static void test_normal(test_context *ctx)
{
  ...
  /** sc = rtems_task_set_scheduler(ctx->low_task_id[6],ctx->scheduler_ids[1],5);
   ...
   rtems_test_assert(sc == RTEMS_SUCCESSFUL);
   */
   create_sema(ctx, &ctx->dpcp_ids[6],1 );
 ...
 sc = rtems_task_wake_after(20);
         if (sc == RTEMS_SUCCESSFUL)
           puts("sleep task on processor 0 ");
                else
          printf("Can't sleep task: %s\n", rtems_status_text(sc));
...
rtems_test_assert(run);
    ...
    if (sc == RTEMS_SUCCESSFUL)
      puts("delete semaphore ");
    else
     printf("Can't delete semaphore: %s\n", rtems_status_text(sc));
    
    sc = rtems_task_delete(ctx->high_task_id[6]);
      if (sc == RTEMS_SUCCESSFUL)
               puts("delete task high ");
          else
                   printf("Can't delete task high: %s\n", rtems_status_text(sc));

 
    sc = rtems_task_delete(ctx->low_task_id[6]);
      if (sc == RTEMS_SUCCESSFUL)
               puts("delete low task ");
          else
                   printf("Can't delete low task: %s\n", rtems_status_text(sc));
