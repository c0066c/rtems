...
static void suspend_critical(rtems_task_argument arg)
{
   ...
   *run_=true;
   ...
   sc = rtems_semaphore_obtain(ctx->dnpp_ids[6], RTEMS_WAIT, RTEMS_NO_TIMEOUT);
   if (sc == RTEMS_SUCCESSFUL)
    puts("obtain semaphore 1 ");
   else
   ...
   rtems_test_assert(sc == RTEMS_SUCCESSFUL);
    if (sc == RTEMS_SUCCESSFUL)
             puts("sleep task low ");
                 ...
                 printf("Can't sleep task low: %s\n", rtems_status_text(sc));

   sc = rtems_semaphore_release(ctx->dnpp_ids[6]);
  if (sc == RTEMS_SUCCESSFUL)
           puts("release semaphore 1 ");
      else
...
static void test_critical(test_context *ctx)
    ...
    create_sema(ctx, &ctx->dnpp_ids[7],2);
    create_sema (ctx,&ctx->dnpp_ids[8],3);
...
*/
    ...
    sc = rtems_semaphore_delete(ctx->dnpp_ids[7]);
     if (sc == RTEMS_SUCCESSFUL)
       puts("Can delete semaphore ");
     else
       printf("Can't delete semaphore: %s\n", rtems_status_text(sc));
     sc= rtems_semaphore_delete(ctx->dnpp_ids[8]);
    if (sc == RTEMS_SUCCESSFUL)
                puts("can delete semaphore2 ");
          else
                      printf("Can't delete semaphore: %s\n", rtems_status_text(sc));
...
static void test_flush_error(test_context *ctx)
{
 rtems_status_code sc;
 rtems_id id;
 rtems_task_priority prio;
