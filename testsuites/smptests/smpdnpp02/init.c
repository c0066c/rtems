...
#include <rtems/score/smplock.h>
const char rtems_test_name[] = "SMPDNPP 02";
#define SCHED_A rtems_build_name(' ', ' ', ' ', 'A')
#define SCHED_B rtems_build_name(' ', ' ', ' ', 'B')
#define SWITCH_EVENT_COUNT 32
#define DNPP_COUNT 32
#define CPU_COUNT 2 
typedef struct {
    uint32_t cpu_index;
...
/**    rtems_test_assert(old_prio == 0);*/
 ...
 static void test_critical(test_context *ctx)
  ...
  rtems_test_assert(sc == RTEMS_SUCCESSFUL);
  create_sema(ctx, &ctx->dnpp_ids[7],6);
  create_sema (ctx,&ctx->dnpp_ids[8],2);
...
#define CONFIGURE_MAXIMUM_SEMAPHORES (DNPP_COUNT + 1)
#define CONFIGURE_MAXIMUM_DNPP_SEMAPHORES DNPP_COUNT
